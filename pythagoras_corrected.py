...
class CorrectedPythagoreanTheorem(Scene):
    def construct(self):
        ...
        midpoint = side_c_line.get_center()
        label_c = (Text("c").scale(0.6).set_color(COLOR_C).move_to(midpoint + UP * 0.25))

        right_angle = RightAngle(side_b_line,side_a_line,length=0.2,color=WHITE)
        # Title
        ...
        self.wait(2)
        self.play(FadeOut(proof_intro_text))
        ...
        label_large_side = Text("Side = a + b").scale(0.6).next_to(large_square, DOWN, buff=0.5)
        ...
        self.wait(2)
        proof_diagram = VGroup(large_square,label_large_side,t1, t2, t3, t4,square_c_inner,label_c_inner)
        self.play(proof_diagram.animate.scale(0.55).to_edge(LEFT, buff = 0.3))

        # Explanatory text for areas
        equation1 = (Text("Area of large square = (a + b)²").scale(0.5).move_to(LEFT*0.5+ UP*0.5).shift(UP * 2))
        self.play(Write(equation1))
        self.wait(2)

        equation2 = Text(f"Area of large square = 4 × (Area of one triangle) + Area of inner square").scale(0.5).next_to(equation1, DOWN, aligned_edge=LEFT).set_color(WHITE)
        self.play(Write(equation2))
        self.wait(2)
        equation3 = Text("(a + b)² = 4 × (1/2 × a × b) + c²").scale(0.5).next_to(equation2, DOWN, aligned_edge=LEFT).set_color(WHITE)
        self.play(Write(equation3))
        self.wait(2)

        equation4 = Text("(a + b)² = 2ab + c²").scale(0.5).next_to(equation3, DOWN, aligned_edge=LEFT).set_color(WHITE)
        self.play(Write(equation4))
        self.wait(2)

        equation5 = Text("a² + 2ab + b² = 2ab + c²").scale(0.5).next_to(equation4, DOWN, aligned_edge=LEFT).set_color(WHITE)
        self.play(Write(equation5))
        self.wait(2)

        equation6 = Text("a² + b² = c²").scale(0.7).next_to(equation5, DOWN, buff=0.5, aligned_edge=LEFT).set_color(COLOR_TEXT)
